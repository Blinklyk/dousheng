...
func (us *RelationService) RelationAction(userId int64, actionVar *request.RelationActionRequest) error {

	actionType := actionVar.ActionType
	// follow operation
	if actionType == "1" {
		err := Follow(userId, actionVar)
		return err
	}

	if actionType == "2" {
		err := CancelFollow(userId, actionVar)
		return err
	}
...
func Follow(userId int64, actionVar *request.RelationActionRequest) error {
	...
	res := tx.Model(&model.Follow{}).Where("user_id = ? AND follow_id = ?", userId, actionVar.ToUserID).Find(&followVar)
	...
	res = tx.Model(&model.Follow{}).Where("user_id = ? AND follow_id = ?", actionVar.ToUserID, userId).First(&toUserFollowVar)
	if res.RowsAffected == 0 {
		status = false
	}
	if res.RowsAffected != 0 {
		...
		if result := tx.Model(&model.Follow{}).Where("user_id = ? AND follow_id = ?", actionVar.ToUserID, userId).Update("status", true); result.RowsAffected == 0 {
			tx.Rollback()
			return errors.New("error: update status in toUser data in follow table")
		}

		if result := tx.Model(&model.Follower{}).Where("user_id = ? AND follower_id = ?", userId, actionVar.ToUserID).Update("status", true); result.RowsAffected == 0 {
			tx.Rollback()
	...
	if res := tx.Model(&model.User{}).Where("id = ?", userId).
...
func CancelFollow(userId int64, actionVar *request.RelationActionRequest) error {
	...
	res := global.App.DY_DB.Model(&model.Follow{}).Where("user_id = ? AND follow_id = ?", userId, actionVar.ToUserID).Find(&followVar)
	...
	if followVar.Status == true {
		if res := tx.Model(&model.Follow{}).Where("user_id = ? AND follow_id = ?", actionVar.ToUserID, userId).Update("status", false); res.RowsAffected == 0 {
			tx.Rollback()
			return errors.New("error in updating the toUser status in follow table" + res.Error.Error())
		}
		if res := tx.Model(&model.Follower{}).Where("user_id = ? AND follower_id = ?", userId, actionVar.ToUserID).Update("status", false); res.RowsAffected == 0 {
			tx.Rollback()
	...
	if res := tx.Model(&model.Follow{}).Where("user_id = ? AND follow_id = ?", userId, actionVar.ToUserID).Delete(&model.Follow{}); res.RowsAffected == 0 {
		tx.Rollback()
		return errors.New("error in deleting record in follow table" + res.Error.Error())
	}

	// 4. delete relation in follower table;
	if res := tx.Model(&model.Follower{}).Where("user_id = ? AND follower_id = ?", actionVar.ToUserID, userId).Delete(&model.Follower{}); res.RowsAffected == 0 {
		tx.Rollback()
